...
void print_usage(const char * argv0) {
    std::cerr << "usage: " << argv0 << " --fp-model original.gguf --tq-model input-tq1_0.gguf --calib text.txt --out calibrated.gguf [options]\n"
              << "\n"
              << "Runs calibration text through llama.cpp, captures matmul input activations, and rewrites TQ1_0 tensors\n"
              << "using an activation-weighted block-output loss approximation. The original dense GGUF is required.\n"
              << "\n"
              << "Options:\n"
              ...
              << "  --chunk-mib N           Tensor chunk size for CUDA reconstruction (default: 512)\n"
              << "  --max-capture-mib N     Skip any single activation tensor larger than this (default: 256)\n"
              << "  --iterations N          Coordinate refinement iterations per TQ block (default: 8)\n"
              << "  --threshold-base X      Initial threshold = X * weighted mean(abs(W)) (default: 0.75)\n"
              << "  --include-regex REGEX   Only reconstruct matching tensor names\n"
              << "  --exclude-regex REGEX   Do not reconstruct matching tensor names\n"
...
void capture_activation_tensor(const ggml_tensor * tensor, activation_stat & stat, capture_state & state) {
    ...
    const uint64_t vectors = static_cast<uint64_t>(n1) * static_cast<uint64_t>(n2) * static_cast<uint64_t>(n3);
        ...
        for (int64_t i2 = 0; i2 < n2; ++i2) {
            for (int64_t i1 = 0; i1 < n1; ++i1) {
                ...
                for (int64_t j = 0; j < n0; ++j) {
                    const size_t off = base + static_cast<size_t>(j) * tensor->nb[0];
                    if (off + elem_size <= data.size()) {
                        const float v = read_scalar_from_bytes(data.data() + off, tensor->type);
                        stat.sumsq[static_cast<size_t>(j)] += static_cast<double>(v) * static_cast<double>(v);
                    }
                }
            }
        }
...
void normalize_importance(const activation_stat & stat, std::vector<float> & importance) {
    importance.resize(static_cast<size_t>(stat.n_features));
    if (stat.samples == 0) {
        std::fill(importance.begin(), importance.end(), 1.0f);
        return;
    }

    double mean = 0.0;
    for (uint64_t i = 0; i < stat.n_features; ++i) {
        const double v = stat.sumsq[static_cast<size_t>(i)] / static_cast<double>(stat.samples);
        importance[static_cast<size_t>(i)] = static_cast<float>(std::max(v, 1.0e-12));
        mean += static_cast<double>(importance[static_cast<size_t>(i)]);
    }
    mean /= static_cast<double>(std::max<uint64_t>(1, stat.n_features));
    const float inv_mean = mean > 0.0 ? static_cast<float>(1.0 / mean) : 1.0f;
    for (float & v : importance) {
        v = std::max(1.0e-8f, v * inv_mean);
    }
}
...
__global__ void refine_tq1_weighted_diag_kernel(
        const uint8_t * input,
        uint8_t * output,
    ...
    const uint64_t feature0 = col_block * TQ_BLOCK_SIZE;

    float w[TQ_BLOCK_SIZE];
    float h[TQ_BLOCK_SIZE];
    int8_t q[TQ_BLOCK_SIZE];

    float weighted_abs = 0.0f;
    float weight_sum = 0.0f;
    for (int i = 0; i < static_cast<int>(TQ_BLOCK_SIZE); ++i) {
        const float v = load_dense_value(input, elem0 + i, input_type);
        const float imp = fmaxf(importance[feature0 + i], 1.0e-8f);
        w[i] = v;
        h[i] = imp;
        weighted_abs += imp * fabsf(v);
        weight_sum += imp;
    }

    const float delta = threshold_base * weighted_abs / fmaxf(weight_sum, 1.0e-8f);
    float weighted_active_abs = 0.0f;
    float active_weight = 0.0f;
    for (int i = 0; i < static_cast<int>(TQ_BLOCK_SIZE); ++i) {
        if (fabsf(w[i]) > delta) {
            q[i] = w[i] > 0.0f ? 1 : -1;
            weighted_active_abs += h[i] * fabsf(w[i]);
            active_weight += h[i];
        } else {
            q[i] = 0;
        }
    ...
    float alpha = active_weight > 0.0f ? weighted_active_abs / active_weight : 0.0f;
    ...
    for (int it = 0; it < iterations; ++it) {
        ...
        alpha = denom > 0.0f ? fmaxf(0.0f, numer / denom) : 0.0f;
...
void refine_tensor_cuda(
    ...
    normalize_importance(stat, importance);

    float * dev_importance = nullptr;
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&dev_importance), importance.size() * sizeof(float)));
    CUDA_CHECK(cudaMemcpy(dev_importance, importance.data(), importance.size() * sizeof(float), cudaMemcpyHostToDevice));
    ...
    while (rows_done < fp_tensor.nrows) {
        ...
        if (!fp_in) {
            CUDA_CHECK(cudaFree(dev_importance));
        ...
        refine_tq1_weighted_diag_kernel<<<static_cast<uint32_t>(grid64), threads>>>(
                dev_input,
                dev_output,
                dev_importance,
                n_blocks,
        ...
        if (!out) {
            CUDA_CHECK(cudaFree(dev_importance));
            throw std::runtime_error("failed to write reconstructed tensor: " + tq_tensor.name);
        }
...
int main(int argc, char ** argv) {
    try {
        ...
        for (const tensor_info & tq_tensor : tq_model.tensors) {
            ...
            stat.sumsq.assign(static_cast<size_t>(stat.n_features), 0.0);
        ...
        for (const tensor_pair & p : refine) {
            std::cerr << "  target " << p.tq->name << " " << dims_to_string(p.tq->dims)
                      << " " << type_name(p.fp->type) << " + activation stats -> TQ1_0\n";
        }
        ...
        if (refine.empty()) {
            throw std::runtime_error("no matching TQ1_0 tensors found to reconstruct");
        }
        ...
        for (size_t i = 0; i < refine.size(); ++i) {
            ...
            if (p.stat->samples == 0) {
                std::cerr << "[" << (i + 1) << "/" << refine.size() << "] reconstruct " << p.tq->name
                          << " (no activation samples; using uniform weights)\n";
            } else {
                std::cerr << "[" << (i + 1) << "/" << refine.size() << "] reconstruct " << p.tq->name
