...
void print_usage(const char * argv0) {
    std::cerr << "usage: " << argv0 << " --fp-model original.gguf --layout-tq-model input-tq1_0.gguf --calib text.txt --out trained-tq1_0.gguf [options]\n"
              << "\n"
              << "Prototype post-training fake-ternary trainer for TQ1_0 tensors. The layout TQ model supplies\n"
              << "GGUF tensor storage only; training starts from original dense teacher weights.\n"
              << "This captures matmul input covariance and runs CUDA STE-style local block-output training.\n"
              << "\n"
              << "Options:\n"
              << "  --layout-tq-model PATH  Existing TQ1_0 GGUF used as output layout (--tq-model is an alias)\n"
...
options parse_args(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--tq-model" || arg == "--layout-tq-model") {
            opt.tq_model = need_value(arg.c_str());
        } else if (arg == "--calib") {
    ...
    if (opt.fp_model.empty() || opt.tq_model.empty() || opt.output_model.empty() || opt.calib_path.empty()) {
        print_usage(argv[0]);
        throw std::runtime_error("--fp-model, --layout-tq-model/--tq-model, --calib and --out are required");
    }
...
void run_calibration(const options & opt, capture_state & state) {
    ...
    if (text.size() < 64ull * 1024ull) {
        std::cerr << "warning: calibration text is only " << text.size()
                  << " bytes; use at least several thousand representative tokens for useful activation fitting\n";
    }
    if (tokens.size() < 512) {
        std::cerr << "warning: calibration text tokenized to only " << tokens.size()
                  << " tokens; this is a smoke test, not enough to train a whole-model TQ1_0 layout\n";
    }
...
int main(int argc, char ** argv) {
    try {
        ...
        std::cerr << "layout:   " << opt.tq_model << "\n";
        std::cerr << "calib:    " << opt.calib_path << "\n";
        std::cerr << "output:   " << opt.output_model << "\n";
        std::cerr << "targets:  " << refine.size() << " TQ1_0 tensors\n";
        if (refine.size() > 64 && opt.include_regex.empty()) {
            std::cerr << "warning: optimizing " << refine.size()
                      << " TQ1_0 tensors is a whole-model ternary run; post-hoc local block training can still collapse LLM output\n"
                      << "         start with --include-regex for a small tensor/layer subset, then expand only after perplexity improves\n";
        }
        for (const tensor_pair & p : refine) {
            std::cerr << "  target " << p.tq->name << " " << dims_to_string(p.tq->dims)
                      << " " << type_name(p.fp->type) << " + fake-ternary training -> TQ1_0\n";
        }
